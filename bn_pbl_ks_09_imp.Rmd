...
```{r uzupełnienia}
obiekt <- sheets_read(ss = "1Gc4gQSm9b4NDTQysiauzW9Jac6yP0oNuFbB8utO4kS4") %>% 
  filter(!is.na(pracownik))

uzupelnienia <- bn_ok %>% 
  filter(X245 %notin% obiekt$X245)

count <- as.data.frame(table(uzupelnienia$redaktor_dzialu))

obiekt_testowy <- uzupelnienia %>% 
  filter(redaktor_dzialu == "BEATAS", DZ_NAZWA == "Historia literatury (grecka starożytna)") %>% 
  select(X080, X245, X650, X655)

obiekt_kolejny <- uzupelnienia %>% 
  filter(grepl("podręcznik", X655, ignore.case = TRUE))

bn_ok_pelne <- bn_ok

bn_ok <- uzupelnienia

obiekt <- dbGetQuery(PBL,
                     "select z.za_url_bn
from IBL_OWNER.pbl_zapisy z
where z.za_uwagi like '%import%2020%'") %>% 
  mutate(ZA_URL_BN = str_extract(ZA_URL_BN, "(?<=alma)(\\d+)"))

uzupelnienia <- bn_ok %>% 
  filter(BN_URL %notin% obiekt$ZA_URL_BN)
...
rocznik_pbl_po_imporcie <- dbGetQuery(PBL,
                                       "select z.za_zapis_id, z.za_status_imp, z.za_uwagi
                                        from pbl_zapisy z
                                        where za_uwagi like '%2020-02-19%'
                                        and z.za_url_bn is not null") %>% 
  mutate(ZA_UWAGI = str_extract(ZA_UWAGI,"(?<=BN: )(.*?)(?=, import)")) %>%
  rename(status = ZA_STATUS_IMP) %>% 
  left_join(.,uzupelnienia %>% select(pracownik = redaktor_dzialu,typ_ksiazki = rodzaj_ksiazki,link = BN_URL, rok, X100, X245, X650, X655, X246, X250, X260, X300, X380, X490, X500, X501, X546, X600, X700, X041, X080,ZA_UWAGI) %>% mutate(link_1 = paste("=HYPERLINK(\"",link,"\";\"link do książki w BN\")",sep = ""), blad_w_imporcie_tytulu = FALSE),by="ZA_UWAGI") %>% 
...
test <- uzupelnienia %>% select(pracownik = redaktor_dzialu,typ_ksiazki = rodzaj_ksiazki,link = BN_URL, rok, X100, X245, X650, X655, X246, X250, X260, X300, X380, X490, X500, X501, X546, X600, X700, X041, X080,ZA_UWAGI) %>% mutate(link_1 = paste("=HYPERLINK(\"",link,"\";\"link do książki w BN\")",sep = ""), blad_w_imporcie_tytulu = FALSE)
...
for (i in x) {
  ...
  final <- out %>%
    filter(podzial %in% ile[[i]]) %>%
    select(-podzial)
  write.xlsx(final, paste("C:/Users/Cezary/Desktop/2009_wspolwydanie_do_importu",i,".xlsx",sep = ""),sheetName = "gotowe")
}
```

bezpiecznik
UWAGA!
Najlpierw należy przeprowadzić import do bazy, a następnie egzekwować poniższą część kodu
...
```{r problemy}
#Iza ma 800 książek, bo prawie 500 wpada do Słowników biograficznych
test <- as.data.frame(table(bn_ok$redaktor_dzialu,bn_ok$czy_automatycznie,bn_ok$DZ_NAZWA)) %>% 
  filter(Var2=="nie") %>% 
  filter(Var1=="IZA") %>% 
  filter(Freq>0)

test2 <- bn_ok %>% 
  filter(redaktor_dzialu=="IZA") %>% 
  filter(czy_automatycznie=="nie") %>% 
  filter(DZ_NAZWA=="Słowniki językowe")

write.xlsx(test2, "C:/Users/Cezary/Desktop/IZA_slowniki_jezykowe.xlsx",sheetName = "arkusz")
...
#rozwiązanie problemu przypisanych książek do IH

iza_slowniki <- sheets_read(ss = "1PPOdOAV6VBKVA0Lf9pmgvWLbSb7_grFWQL6eJhh36V8")

test <- iza_slowniki %>% 
  filter(X655!="\\7$aBiografia$2DBN")

slowniki_jezykowe <- iza_slowniki %>% 
  filter(DZ_NAZWA=="Słowniki językowe") %>% 
  filter(grepl("811\\.162",X080)) %>% #to wtedy musi mieć słownik
#jest 119
  filter(grepl("słownik",X655,ignore.case = TRUE))
#zostanie 63

slowniki_biograficzne <- iza_slowniki %>% 
  #filter(DZ_NAZWA=="Słowniki biograficzne") %>% 
  #jest 481
  filter(grepl("929\\-052",X080)) %>% #to wtedy musi mieć to, co poniżej
  filter(grepl("Słownik biograficzny|Biografia",X655)&grepl("pisarz|litera|aktor|scenograf|nauczyciel|akademi|nauk|filolog|lit|kryty",X655,ignore.case = TRUE))
#zostanie 2
...
count <- as.data.frame(table(bn_ok$dziedzina_PBL))

test <- bn_ok %>% 
  filter(grepl("82",X080))

# zaimportować współwydania 2009 i przejrzeć pliki do zrobienia na później (trzeba uwzględnić to, że zmieniły się regulacje ze względu na słowniki językowe i biograficzne)
