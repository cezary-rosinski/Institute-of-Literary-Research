...
options(scipen = 999)
pacman::p_load(googlesheets4,zoo,openxlsx,stringr,splitstackshape,plyr,dplyr,sqldf,stringdist,fuzzyjoin,data.table,svMisc,tidyverse,RJDBC,xlsx,magrittr,jsonlite,openxlsx,xml2,XML)
trim <- function (x) gsub("^\\s+|\\s+$", "", x)
pbl_hp <- read.csv2("C:/Users/Cezary/Downloads/pbl_subject_headings.csv", encoding = "Windows-1250", header = TRUE, stringsAsFactors = FALSE)
pbl_marc_roles <- sheets_read(ss = "1iQ4JGo1hLWZbQX4u2kB8LwqSWMLwicFfBsVFc9MNUUk", sheet = "pbl-marc") %>% 
  filter(!is.na(MARC_ABBREVIATION)) %>% 
  select(PBL_NAZWA, MARC_ABBREVIATION) %>% 
  mutate(MARC_ABBREVIATION = paste("$4", MARC_ABBREVIATION, sep = "")) %>% 
  group_by(PBL_NAZWA) %>% 
  mutate(MARC_ABBREVIATION = paste(MARC_ABBREVIATION,collapse = "|")) %>% 
  ungroup() %>% 
  unique()
...
#   select(pbl_id,BN_name,viaf) %>%
#   filter(!is.na(viaf)) %>%
#   mutate(viaf = paste("viaf",str_extract(viaf,"\\d+"),sep = "")) %>%
#   group_by(pbl_id) %>%
#   mutate(viaf = paste(unique(viaf),collapse = "~")) %>%
#   ungroup() %>%
#   unique()
# 
...
```{r addictional info about authors from viaf}
# x <- 1:nrow(pbl_viaf)
# viaf_name_dates <- data.frame(stringsAsFactors = FALSE)
# for (i in x) {
#   progress(match(i,x), max.value = length(x))
#   url <- paste("http://viaf.org/viaf/",str_extract(pbl_viaf$viaf[i],"\\d+"),"/viaf.xml",sep = "")
#   xml <- xmlParse(url)
#   tryCatch({
#     name <- setNames(xmlToDataFrame(nodes = getNodeSet(xml, "//ns1:VIAFCluster/ns1:mainHeadings/ns1:data/ns1:text")),"name")
#     name <- paste(unlist(name), collapse = "~")
#     }, error=function(e){
#       name <<- "Brak danych (CR)"
#       })
#     tryCatch({
#     birthDate <- setNames(xmlToDataFrame(nodes = getNodeSet(xml, "//ns1:birthDate")),"birthDate")
#     birthDate <- paste(unlist(birthDate), collapse = "~")
#     }, error=function(e){
#       birthDate <<- "Brak danych (CR)"
#       })
#     tryCatch({
#     deathDate <- setNames(xmlToDataFrame(nodes = getNodeSet(xml, "//ns1:deathDate")),"deathDate")
#     deathDate <- paste(unlist(deathDate), collapse = "~")
#     }, error=function(e){
#       deathDate <<- "Brak danych (CR)"
#       })
#   iteration <- pbl_viaf %>% slice(i) %>% 
#     bind_cols(name=name,birthDate=birthDate,deathDate=deathDate)
#   viaf_name_dates <- rbind(viaf_name_dates,iteration)
# }
# write.xlsx(viaf_name_dates, "C:/Users/Cezary/Desktop/viaf_name_dates.xlsx",sheetName = "pbl_viaf")

# pbl_viaf_names_dates <- pbl_viaf %>% 
#   left_join(viaf_name_dates,by = c("pbl_id","viaf")) %>% 
#   mutate(`$a` = str_extract(BN_name,"(.*?)(?=\\|.*$)"),
#          `$a` = str_extract(BN_name,"(.*?)(?=\\ \\(\\d.*$)"),
#          `$a` = ifelse(is.na(`$a`),str_extract(BN_name,"(.*?)(?=\\|.*$)"),as.character(`$a`)),
#          `$d` = ifelse(birthDate==0&deathDate==0,"",
#                        ifelse(deathDate==0,paste(substr(birthDate,1,4),"- ",sep = ""),
#                               ifelse(birthDate==0,paste(" -",substr(deathDate,1,4),sep = ""),paste(substr(birthDate,1,4),"-",substr(deathDate,1,4),sep = ""))))) %>% 
#   select(pbl_id,`$a`,`$d`,viaf)
# write.xlsx(pbl_viaf_names_dates, "C:/Users/Cezary/Desktop/pbl_viaf_names_dates.xlsx",sheetName = "pbl_viaf")

pbl_viaf_names_dates <- sheets_read(ss = "1S_KsVppXzisd4zlsZ70rJE3d40SF9XRUO75no360dnY")

```


```{r pbl books}
pbl_books <- dbGetQuery(PBL,
...
pbl_books <- merge(pbl_books,pbl_viaf_names_dates,by.x = "tworca_id", by.y = "pbl_id", all.x = TRUE) 
pbl_books <- merge(pbl_books, pbl_marc_roles, by.x = "funkcja_osoby", by.y = "PBL_NAZWA", all.x = TRUE) %>%
  arrange(rekord_id)
```

```{r pbl reviews and books}
pbl_reviews_books <- dbGetQuery(PBL,
                                "select z.za_zapis_id \"rec_id\", a.am_nazwisko \"rec_a_naz\", a.am_imie \"rec_a_im\", z.za_tytul \"rec_tyt\", zrr.zrr_tytul \"rec_czas_tyt\", zrr.zrr_miejsce_wydania \"rec_czas_miejsc\", z.za_zrodlo_rok \"rec_rok\", z.za_zrodlo_nr \"rec_nr\", z.za_zrodlo_str \"rec_str\", 
                                z2.za_zapis_id \"ks_id\", a2.am_nazwisko \"ks_a_naz\", a2.am_imie \"ks_a_im\", z2.za_tytul \"ks_tyt\", w.wy_nazwa \"ks_wyd_nazw\", w.wy_miasto \"ks_wyd_miejsc\", z2.za_ro_rok
                                from pbl_zapisy z
  ...
  mutate(rec_a_naz = ifelse(is.na(rec_a_naz),"[no author]",as.character(rec_a_naz)),
         rec_tyt = ifelse(is.na(rec_tyt),"[no title]",as.character(rec_tyt)),
         ks_a_naz = ifelse(is.na(ks_a_naz),"[no author]",as.character(ks_a_naz))) %>% 
  unite("rec_autor", rec_a_naz:rec_a_im,sep = " ",na.rm = TRUE) %>% 
  unite("rec_czas_miejsc", rec_czas_miejsc:rec_rok,sep = " ",na.rm = TRUE) %>%
  unite("rec", rec_autor:rec_str,sep = ", ",na.rm = TRUE) %>%
  unite("ks_autor", ks_a_naz:ks_a_im,sep = " ",na.rm = TRUE) %>% 
  unite("ks_wyd_miejsc", ks_wyd_miejsc:ZA_RO_ROK,sep = " ",na.rm = TRUE) %>%
  unite("ks", ks_autor:ks_wyd_miejsc,sep = ", ",na.rm = TRUE) #%>% 
  # mutate(rec_id = paste("pl",sprintf("%09d", rec_id),sep = "")) %>% 
  # mutate(ks_id = paste("pl",sprintf("%09d", ks_id),sep = ""))

```


```{r creating pbl person authority file}
...
#X100
X100 <- pbl_books %>%
  select(rekord_id,rodzaj_zapisu,tworca_nazwisko,tworca_imie,autor_nazwisko,autor_imie,`$a`,`$d`,`$0`) %>%
  unique() %>%
  mutate(tworca_nazwisko = ifelse(rodzaj_zapisu=="książka twórcy (podmiotowa)"&!is.na(tworca_nazwisko)&grepl(",",tworca_nazwisko)&tworca_imie=="*",as.character(autor_nazwisko),as.character(tworca_nazwisko)),
         ...
         X100 = ifelse(is.na(tworca_nazwisko),NA,
                       ifelse(rodzaj_zapisu=="książka twórcy (podmiotowa)"&nchar(paste(tworca_nazwisko,tworca_imie,sep = ", "))==nchar(`$a`)&as.character(str_extract_all(paste(tworca_nazwisko,tworca_imie,sep = ", "),"[:upper:]"))==as.character(str_extract_all(`$a`,"[:upper:]")), paste("1\\$a",`$a`,"$d",`$d`,"$4aut$0",`$0`,sep = ""),
                              ifelse(rodzaj_zapisu=="książka twórcy (podmiotowa)",paste("1\\$a",tworca_nazwisko,", ",tworca_imie,"$d",`$d`,"$4aut$0",`$0`,sep = ""),
...
X245 <- pbl_books %>%
  ...
  unique() %>%
  mutate(tytul_ok = ifelse(is.na(tytul),"",str_remove(paste("10$a",ifelse(substr(tytul,nchar(tytul),nchar(tytul))=="]",trim(str_replace(tytul,"(^.*?)(([\\.!?] {0,1})\\[.*?$)","\\1")),
  ...
  mutate(X245 = paste(tytul_ok,autor," ; ",wspoltworcy," ; ",ZA_INSTYTUCJA,".", sep = ""),
         X245 = str_remove(trim(str_replace_all(X245,"( ;  ; )(\\.)|( ;   ; )(\\.)|( ; )(\\.)|","\\2")),"^\\.$")) %>%
...
X264 <- pbl_books %>%
  ...
  unique() %>%
  mutate(X264 = ifelse(is.na(wydawnictwo)&is.na(miejscowosc),"1\\$aS.l. : $bs.n.",paste("1\\$a",miejscowosc," : $b",wydawnictwo,sep = ""))) %>%
...
#X600
X600 <- pbl_books %>%
  select(rekord_id,rodzaj_zapisu,tworca_nazwisko,tworca_imie,`$a`,`$d`,`$0`) %>%
  unique() %>%
  ...
  mutate(X600 = trim(ifelse(nchar(paste(tworca_nazwisko,tworca_imie,sep = ", "))==nchar(`$a`)&as.character(str_extract_all(paste(tworca_nazwisko,tworca_imie,sep = ", "),"[:upper:]"))==as.character(str_extract_all(`$a`,"[:upper:]")), paste("14$a",`$a`,"$d",`$d`,"$0",`$0`,sep = ""),
                       ifelse(tworca_imie!="",paste("14$a",tworca_nazwisko,", ",tworca_imie,"$d",`$d`,"$0",`$0`,sep = ""),paste("14$a",tworca_nazwisko,sep = ""))))) %>%
...
X630b <- pbl_books %>%
  ...
  mutate(dzial = str_replace(dzial, "(.*?)( - [:upper:]$)","\\1")) %>% 
  unique() %>% 
  inner_join(.,pbl_hp %>% select(HP_NAZWA, MARC_FIELD) %>% filter(MARC_FIELD=="4.63"),by = c("dzial"="HP_NAZWA")) %>% 
  mutate(X630 = trim(paste("\\4$a", dzial, sep = ""))) %>% 
  select(rekord_id,X630) %>%
  group_by(rekord_id) %>%
  mutate(X630 = paste(X630,collapse = "|")) %>%
  ungroup() %>%
...
X630 <- X630a %>% 
  bind_rows(X630b) %>% 
  group_by(rekord_id) %>% 
  mutate(X630 = paste(unique(X630), collapse = "|")) %>% 
  ungroup() %>% 
  unique() %>% 
  full_join(pbl_books %>% select(rekord_id) %>% unique(),by = "rekord_id") %>%
  arrange(rekord_id)
...
X650b <- pbl_books %>%
  ...
  mutate(dzial = str_replace(dzial, "(.*?)( - [:upper:]$)","\\1")) %>% 
  unique() %>% 
  inner_join(.,pbl_hp %>% select(HP_NAZWA, MARC_FIELD) %>% filter(MARC_FIELD=="4.65"),by = c("dzial"="HP_NAZWA")) %>% 
  mutate(X650 = trim(paste("04$a", dzial, sep = ""))) %>% 
  select(rekord_id,X650) %>%
  group_by(rekord_id) %>%
  mutate(X650 = paste(X650,collapse = "|")) %>%
  ungroup() %>%
...
X655 <- pbl_books %>%
  ...
  unique() %>%
  group_by(rekord_id) %>%
  mutate(X655 = ifelse(is.na(tytul),"",trim(paste(unlist(str_extract_all(tytul,"(?<!^.)(?<=\\[)(.*?)(?=\\])")),collapse = "|")))) %>%
  ungroup() %>%
...
#X700
X700 <- pbl_books %>%
  select(rekord_id,funkcja_osoby,wspoltworca_nazwisko,wspoltworca_imie,MARC_ABBREVIATION) %>%
  unique() %>%
  mutate(X700 = trim(ifelse(is.na(wspoltworca_nazwisko),"",paste("1\\$a",wspoltworca_nazwisko,", ",wspoltworca_imie,"$e",funkcja_osoby, ifelse(!is.na(MARC_ABBREVIATION),as.character(MARC_ABBREVIATION),""), sep = "")))) %>%
...
count <- as.data.frame(table(X700$rekord_id))
...
#X856
X856 <- pbl_reviews_books %>% 
  select(rekord_id=ks_id,rec_id,rec) %>% 
  mutate(X856 = paste("42$uhttps://vufind-dev.ucl.cas.cz/Record/","pl",sprintf("%09d", rec_id),"$y","review: ",rec,"$4N",sep = "")) %>% 
  select(rekord_id,X856) %>% 
  group_by(rekord_id) %>%
  mutate(X856 = paste(X856,collapse = "|")) %>%
  ungroup() %>%
  unique() %>%
  right_join(pbl_books %>% select(rekord_id) %>% unique(),by = "rekord_id") %>%
  arrange(rekord_id)
...
pbl_articles <- merge(pbl_articles,pbl_viaf_names_dates,by.x = "tworca_id", by.y = "pbl_id", all.x = TRUE)
pbl_articles <- merge(pbl_articles, pbl_marc_roles, by.x = "funkcja_osoby", by.y = "PBL_NAZWA", all.x = TRUE) %>%
  arrange(rekord_id)
...
X100 <- pbl_articles %>%
  select(rekord_id,rodzaj_zapisu,tworca_nazwisko,tworca_imie,autor_nazwisko,autor_imie,`$a`,`$d`,`$0`) %>%
  unique() %>%
  mutate(tworca_nazwisko = ifelse(rodzaj_zapisu %in% c("adaptacja utworów", "antologia w czasopiśmie", "listy", "proza", "tekst paraliteracki twórcy", "twórczość pozaliteracka", "wiersz")&!is.na(tworca_nazwisko)&grepl(",",tworca_nazwisko)&tworca_imie=="*",as.character(autor_nazwisko),as.character(tworca_nazwisko)),
         tworca_imie = ifelse(rodzaj_zapisu %in% c("adaptacja utworów", "antologia w czasopiśmie", "listy", "proza", "tekst paraliteracki twórcy", "twórczość pozaliteracka", "wiersz")&!is.na(tworca_nazwisko)&tworca_nazwisko==autor_nazwisko&tworca_imie=="*",as.character(autor_imie),as.character(tworca_imie)),
         X100 = ifelse(is.na(tworca_nazwisko),NA,
                       ifelse(rodzaj_zapisu %in% c("adaptacja utworów", "antologia w czasopiśmie", "listy", "proza", "tekst paraliteracki twórcy", "twórczość pozaliteracka", "wiersz")&nchar(paste(tworca_nazwisko,tworca_imie,sep = ", "))==nchar(`$a`)&as.character(str_extract_all(paste(tworca_nazwisko,tworca_imie,sep = ", "),"[:upper:]"))==as.character(str_extract_all(`$a`,"[:upper:]")), paste("1\\$a",`$a`,"$d",`$d`,"$4aut$0",`$0`,sep = ""),
                              ifelse(rodzaj_zapisu %in% c("adaptacja utworów", "antologia w czasopiśmie", "listy", "proza", "tekst paraliteracki twórcy", "twórczość pozaliteracka", "wiersz"),paste("1\\$a",tworca_nazwisko,", ",tworca_imie,"$d",`$d`,"$4aut$0",`$0`,sep = ""),
                                     ifelse(!is.na(autor_nazwisko),paste("1 $a",autor_nazwisko,", ",autor_imie,"$4aut",sep = ""),NA))))) %>%
  select(rekord_id,X100) %>%
  group_by(rekord_id) %>%
  mutate(X100 = paste(X100,collapse = "|")) %>%
...
#X600
X600 <- pbl_articles %>%
  select(rekord_id,ZA_TYPE,tworca_nazwisko,tworca_imie,`$a`,`$d`,`$0`) %>%
  unique() %>%
  filter(ZA_TYPE=="IZA") %>%
...
#X856
X856 <- pbl_reviews_books %>% 
  select(rekord_id=rec_id,ks_id,ks) %>% 
  mutate(X856 = paste("42$uhttps://vufind-dev.ucl.cas.cz/Record/","pl",sprintf("%09d", ks_id),"$y","review of: ",ks,"$4N",sep = "")) %>% 
  select(rekord_id,X856) %>% 
  group_by(rekord_id) %>%
  mutate(X856 = paste(X856,collapse = "|")) %>%
  ungroup() %>%
  unique() %>%
  right_join(pbl_articles %>% select(rekord_id) %>% unique(),by = "rekord_id") %>%
  arrange(rekord_id)
...
pbl_marc_articles <- X005 %>%
  ...
  cbind(.,LDR,X040,X001) %>%
  select(noquote(order(colnames(.))))
#write.csv2(pbl_marc_articles, file = "C:/Users/Cezary/Desktop/pbl_marc_articles.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
