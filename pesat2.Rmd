...
pbl_books <- merge(pbl_books,pbl_viaf,by.x = "tworca_id", by.y = "pbl_id", all.x = TRUE)
pbl_books <- pbl_books %>%
  mutate(tworca_imie = ifelse(is.na(tworca_id),NA,
                              ifelse(is.na(viaf),as.character(tworca_imie),paste(tworca_imie,"$0",viaf,sep = "")))) %>%
  arrange(rekord_id)
```
...
pbl_articles <- merge(pbl_articles,pbl_viaf,by.x = "tworca_id", by.y = "pbl_id", all.x = TRUE)
