...
pbl_books <- read.csv2("C:/Users/Cezary/Desktop/cz_books.csv", encoding = "Windows-1250", header = TRUE, stringsAsFactors = FALSE)
#empty data.frame
...
X630 <- pbl_books %>%
  ...
  inner_join(.,pbl_hp %>% filter(MARC_FIELD=="4.63"),by = c("dzial"="HP_NAZWA")) %>%
...
#X650
X650 <- pbl_books %>%
  select(rekord_id,HP_NAZWA,KH_NAZWA,dzial) %>%
  unique() %>% 
  inner_join(.,pbl_hp %>% filter(MARC_FIELD=="4.65"|MARC_FIELD=="24.65"),by = c("HP_NAZWA","KH_NAZWA")) %>%
  inner_join(.,pbl_hp %>% filter(MARC_FIELD=="4.65"|MARC_FIELD=="24.65"),by = c("dzial"="HP_NAZWA")) %>%
