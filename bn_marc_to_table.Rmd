...
data_rok <- data_full %>%
  ...
  fill(rok,.direction = "up") %>%
  ungroup() %>%
  filter(rok == 2011) %>%#TUTAJ
  select(-rok)
data_rok_table <- dcast(data_rok, id ~ field, value.var="content")
write.csv2(data_rok_table, file = "C:/Users/User/Desktop/bn_ks_2011.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
```
