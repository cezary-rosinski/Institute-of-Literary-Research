...
bn_ks <- read.csv2("C:/Users/Cezary/Downloads/bn_ks_2010.csv", encoding = "UTF-8", header = TRUE, stringsAsFactors = FALSE) %>%
...
```{r problemy2}
bn_ok <- bn_ok %>% 
  bind_rows(na_pozniej)
count <- as.data.frame(table(bn_ok$dziedzina_PBL,bn_ok$czy_automatycznie,bn_ok$rodzaj_ksiazki)) %>% 
  filter(Freq!=0,
         Var2=="nie") %>% 
  arrange(-Freq)
```
...
write.csv2(na_pozniej, "C:/Users/Cezary/Desktop/bn_2010_ks_na_pozniej.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
...
#aktualizacja - wpisz właściwy rok w ścieżkę pliku
write.csv2(automatyczny_IOK, "C:/Users/Cezary/Desktop/imp_2010_automatyczne_podmiotowe.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
...
for (i in x) {
  ...
  final <- out %>%
    ...
    select(-podzial)
  write.xlsx(final, paste("C:/Users/Cezary/Desktop/2010_podmiotowa_do_importu",i,".xlsx",sep = ""),sheetName = "gotowe")
}
```
...
for (i in x) {
  ...
  final <- out %>%
    filter(podzial %in% ile[[i]]) %>%
    select(-podzial)
  write.xlsx(final, paste("C:/Users/Cezary/Desktop/2010_przedmiotowa_do_importu",i,".xlsx",sep = ""),sheetName = "gotowe")
}
```
...
for (i in x) {
  ...
  final <- out %>%
    filter(podzial %in% ile[[i]]) %>%
    select(-podzial)
  write.xlsx(final, paste("C:/Users/Cezary/Desktop/2010_antologie_do_importu",i,".xlsx",sep = ""),sheetName = "gotowe")
}
```
...
for (i in x) {
  ...
  final <- out %>%
    filter(podzial %in% ile[[i]]) %>%
    select(-podzial)
  write.xlsx(final, paste("C:/Users/Cezary/Desktop/2010_wspolwydanie_do_importu",i,".xlsx",sep = ""),sheetName = "gotowe")
}
```
...
nadpisanie_zapisow_IOK <- read.csv2("C:/Users/Cezary/Desktop/imp_2010_automatyczne_podmiotowe.csv", encoding = "UTF-8", header = TRUE, stringsAsFactors = FALSE)
...
#aktualizacja - wpisz właściwy rok w ścieżkę pliku
write.xlsx(nadpisanie_zapisow_IOK, "C:/Users/Cezary/Desktop/2010_nadpisanie_zapisow_IOK.xlsx",sheetName = "ZA_STATUS_IMP")
...
#aktualizacja - wpisz właściwy rok w ścieżkę pliku
write.xlsx(rocznik_pbl_po_imporcie, "C:/Users/Cezary/Desktop/ks_BN_2010_listy_pracowników.xlsx",sheetName = "lista_ksiazek")
...
```{r problemy}
#skierować do na później większą liczbę rekordów - ogarnąć, jak to zrobić

#automatyczne sugerowanie działu na podstawie 655 i 650 - lepiej nie usuwać tego, co powoduje błędy, tylko jakoś to włączyć do wyszukiwania
