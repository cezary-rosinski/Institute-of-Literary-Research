...
jdbcDriver =JDBC("oracle.jdbc.OracleDriver",classPath="C:/Users/Nikodem/Downloads/ojdbc6.jar")
PBL <- dbConnect(jdbcDriver, "jdbc:oracle:thin:@//pbl.ibl.poznan.pl:1521/xe", "IBL_SELECT", "CR333444")

#wgranie zbiorów książek
#aktualizacja - wprowadź właściwą ścieżkę pliku (zmień rok)
bn_ks <- read.csv2("C:/Users/Nikodem/Downloads/bn_ks_2011.csv", encoding = "UTF-8", header = TRUE, stringsAsFactors = FALSE) %>%
  mutate(rok = str_sub(X008,8,11)) %>% 
  #aktualizacja - wprowadź do filtra właściwy rok
  filter(rok == 2011,
...
#write.csv2(bn_ok, "C:/Users/Nikodem/Desktop/bn_ok_2009.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
```

```{r wyznaczenie zbioru do uzupełnienia w przyszłości}
#bezpiecznik
...
write.csv2(na_pozniej, "C:/Users/Nikodem/Desktop/bn_2011_ks_na_pozniej.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
...
#aktualizacja - wpisz właściwy rok w ścieżkę pliku
write.csv2(automatyczny_IOK, "C:/Users/Nikodem/Desktop/imp_2011_automatyczne_podmiotowe.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
...
for (i in x) {
  ...
  final <- out %>%
    ...
    select(-podzial)
  write.xlsx(final, paste("C:/Users/Nikodem/Desktop/2011_podmiotowa_do_importu",i,".xlsx",sep = ""),sheetName = "gotowe")
}
```
...
for (i in x) {
  ...
  final <- out %>%
    filter(podzial %in% ile[[i]]) %>%
    select(-podzial)
  write.xlsx(final, paste("C:/Users/Nikodem/Desktop/2011_przedmiotowa_do_importu",i,".xlsx",sep = ""),sheetName = "gotowe")
}
```
...
for (i in x) {
  ...
  final <- out %>%
    filter(podzial %in% ile[[i]]) %>%
    select(-podzial)
  write.xlsx(final, paste("C:/Users/Nikodem/Desktop/2011_antologie_do_importu",i,".xlsx",sep = ""),sheetName = "gotowe")
}
```
...
for (i in x) {
  ...
  final <- out %>%
    filter(podzial %in% ile[[i]]) %>%
    select(-podzial)
  write.xlsx(final, paste("C:/Users/Nikodem/Desktop/2011_wspolwydanie_do_importu",i,".xlsx",sep = ""),sheetName = "gotowe")
}
```
...
nadpisanie_zapisow_IOK <- read.csv2("C:/Users/Nikodem/Desktop/imp_2011_automatyczne_podmiotowe.csv", encoding = "UTF-8", header = TRUE, stringsAsFactors = FALSE)
...
#aktualizacja - wpisz właściwy rok w ścieżkę pliku
write.xlsx(nadpisanie_zapisow_IOK, "C:/Users/Nikodem/Desktop/2011_nadpisanie_zapisow_IOK.xlsx",sheetName = "ZA_STATUS_IMP")
...
#aktualizacja - wpisz właściwy rok w ścieżkę pliku
write.xlsx(rocznik_pbl_po_imporcie, "C:/Users/Nikodem/Desktop/ks_BN_2011_listy_pracowników.xlsx",sheetName = "lista_ksiazek")
...
write.xlsx(pbl_bn_viaf_import, "C:/Users/Nikodem/Desktop/relacje_osobowe_pbl_bn.xlsx",sheetName = "pbl-bn")
...
```{r problemy}
# Ewa - jeśli w uwagach jest w 500 tyt oryginału, a później info o pseudonimie, to info nie jest importowane - naprawić dla podmiotowych
# 
# Paulina
# - dziwne trzyosobowe spółki autorskie (chociaż mniej niż poprzednio)
# - nagminne przypisywanie do mnie antologii, w oparciu o nazwisko na B (chociaż jest "et al.", a BN hasłuje jako antologię)
# - zawsze się w oparciu o "J" przypisze do mnie jakiś Jan Paweł II oraz Jerzy Popiełuszko (tym razem nie pod "Uszko Jerzy", tylko nowy autor na J)
# Z błędów nowych:
# - z importu uzupełniającego 2009 oraz z importu 2010 pojawiło się u mnie sporo książek dot. prawa, głównie komentarzy do jakichś kodeksów (pewnie ze wzgl. na hasło interpretacja - co z tego, że przepisów)
# 
# Pomysły 
# - wyprostować 100 i 245 - czy się da?
# - czy z informacji o pseudonimie i nazwisku właściwym w uwagach coś można wyciągnąć
# - sprawdzić statystycznie, które informacje są drukowane najczęściej w uwagach
