...
record_people[] <- lapply(record_people, gsub, pattern='~\\d+|~\\:|~\\,', replacement='')
record_people[] <- lapply(record_people, gsub, pattern="~|'", replacement='')
record_people[] <- lapply(record_people, gsub, pattern="- ", replacement='-')
record_people[] <- lapply(record_people, gsub, pattern=" +", replacement=' ')
...
kartoteka_osob <- sheets_read(ss = "1Y8Y_pfkuKiv5npL6QJAXWbDO6twIJqsK3ArvaokjFkU", sheet = "samizdat",col_types = "c") %>% 
  ...
  mutate(name_form = str_remove_all(name_form, "~\\d+|~\\:|~\\,"),
         name_form = str_remove_all(name_form, "~|'"),
         name_form = str_replace_all(name_form, "- ", "-"),
         name_form = str_replace_all(name_form, " +", " "))
...
nodegoat_people <- record_people %>% 
  ...
  ungroup() %>% 
# test <- nodegoat_people %>% 
#   filter(grepl("\\|", Name)&!grepl("\\|", Given_Name))
  group_by(id_osoby) %>% 
  ...
  arrange(id_osoby,group_id) %>% 
  unique() %>% 
  mutate(Death = ifelse(Death == "", NA, as.character(Death))) %>% 
  group_by_at(vars(Index_Name:Numeration)) %>% 
  mutate(group_id = paste(unique(na.omit(group_id)),collapse = "|"),
         Project_ID = paste(unique(na.omit(Project_ID)),collapse = "|"),
         id_osoby = paste(unique(na.omit(id_osoby)),collapse = "|"),
         name_form_id = paste(unique(na.omit(name_form_id)),collapse = "|"),
         Numeration_arabic = paste(unique(na.omit(Numeration_arabic)),collapse = "|"),
         Birth = paste(unique(na.omit(Birth)),collapse = "|"),
         Death = paste(unique(na.omit(Death)),collapse = "|")) %>% 
  ungroup() %>% 
  arrange(id_osoby,group_id) %>% 
  unique() %>% 
  mutate(Project_ID = sprintf("%05d", 1:n()))
nodegoat_people[] <- lapply(nodegoat_people, gsub, pattern=" +", replacement=' ')
nodegoat_people <- nodegoat_people %>% 
  mutate(Unknown_Name_Source = Addition_to_Name,
         Addition_to_Name = ifelse(grepl("\\(smz\\.\\)|\\(sam\\.\\)", Addition_to_Name), "", as.character(Addition_to_Name))) %>% 
  mutate(Other_Name_Form = ifelse(Other_Name_Form == "", "; ", as.character(Other_Name_Form))) %>% 
  cSplit("Other_Name_Form",sep = "; ",direction = "long") %>% 
  unique() %>% 
  mutate(Other_Name_Form = str_remove(Other_Name_Form, ";"))
x <- 1:nrow(nodegoat_people)
for (i in x) {
  progress(match(i,x), max.value = length(x))
  if (str_replace_all(str_to_lower(nodegoat_people$Index_Name[i]), "\\W", "") == str_replace_all(str_to_lower(nodegoat_people$Other_Name_Form[i]), "\\W", "")) {
    nodegoat_people$Other_Name_Form[i] <- ""
  }
}
nodegoat_people <- nodegoat_people %>% 
  group_by(Project_ID) %>% 
  mutate(Other_Name_Form = paste(Other_Name_Form, collapse = "; ")) %>% 
  ungroup() %>% 
  unique() %>% 
  mutate(Other_Name_Form = str_remove(Other_Name_Form, "; $"),
         Index_Name = ifelse(grepl(",", Index_Name), as.character(Index_Name), 
                             ifelse(str_detect(Index_Name, "^\\p{Ll}| \\p{Ll}"), as.character(Index_Name), str_replace(Index_Name, " ", ", "))))

x <- 2:nrow(nodegoat_people)
j <- 1
for (i in x) {
  progress(match(i,x), max.value = length(x))
  if (str_extract(nodegoat_people$id_osoby[i], ".*?(?=\\||$)") == str_extract(nodegoat_people$id_osoby[i-1], ".*?(?=\\||$)")) {
    nodegoat_people$Project_ID[i] <- sprintf("%05d",j)
  } else {
    j <- j + 1
    nodegoat_people$Project_ID[i] <- sprintf("%05d",j)
  }
}
nodegoat_people$Name[grepl("papież", nodegoat_people$Index_Name)] <- ""
nodegoat_people$Given_Name[grepl("papież", nodegoat_people$Index_Name)] <- ""

#viaf enrichment
nodegoat_people$LOD_ID <- NA
nodegoat_people$viaf_name <- NA
y <- 1:nrow(nodegoat_people)
i <- 2918
for (i in y) {
  progress(match(i,y), max.value = length(y))
  if (nchar(nodegoat_people$Index_Name[i])>=8) {
    url <- paste("http://viaf.org/viaf/search?query=local.personalNames%20all%20%22", str_replace_all(paste(nodegoat_people$Index_Name[i], nodegoat_people$Birth[i], nodegoat_people$Death[i], sep = " ")," ", "%20"), "&sortKeys=holdingscount&recordSchema=BriefVIAF", sep = "")
    remDr$navigate(url)
    tryCatch({
      list_of_people <- remDr$findElements(using = 'css selector', '#resultsList a')
      list_of_people <- sapply(list_of_people, function(x) x$getElementText())
      list_of_people <- as.character(list_of_people)
      z <- 1:length(list_of_people)
      for (k in z) {
        list_of_people[k] <- strsplit(list_of_people[k], "\n")[[1]][1]
      }
      lkj <- as.matrix(stringdistmatrix(a=paste(nodegoat_people$Index_Name[i], nodegoat_people$Birth[i], nodegoat_people$Death[i], sep = " "),b=list_of_people, method = "jw"))
      tekst <- strsplit(list_of_people[match(min(lkj),lkj)], "\n")[[1]][1]
      enter <- remDr$findElement(using = "partial link text", tekst)
      enter$sendKeysToElement(sendKeys = list(key = "enter"))
      nodegoat_people$LOD_ID[i] <- str_remove(str_replace(as.character(remDr$getCurrentUrl()),"https","http"),"\\#.*$")
      viaf_name <- remDr$findElement(using = 'css selector', "#nameEntry1")
      viaf_name <- as.character(viaf_name$getElementText())
      nodegoat_people$viaf_name[i] <- viaf_name
      }, error=function(e){
        nodegoat_people$LOD_ID[i] <<- "Brak danych (CR)"
        nodegoat_people$viaf_name[i] <- "Brak danych (CR)"
      })
  }
}

#po viafie
nodegoat_people <- read.csv2("C:/Users/Cezary/Downloads/nodegoat_people_test.csv", encoding = "UTF-8", header = TRUE, stringsAsFactors = FALSE) 

count <- as.data.frame(table(nodegoat_people$LOD_ID))

nodegoat_people <- merge(nodegoat_people, count, by.x = "LOD_ID", by.y = "Var1", all.x = TRUE)

te_same_osoby <- nodegoat_people %>% 
  filter(Freq == 2) %>% 
  mutate(pierwsza_litera = substr(Index_Name, 1, 1))

nodegoat_people <- nodegoat_people %>% 
  filter(Freq != 2) %>% 
  select(-Freq)

te_same_osoby$te_same_osoby <- cumsum(!duplicated(te_same_osoby[18:20]))
te_same_osoby[te_same_osoby==""]<-NA

te_same_osoby <- te_same_osoby %>% 
  group_by(te_same_osoby) %>%
  summarise_each(funs(paste(unique(na.omit(.)), collapse = "|"))) %>% 
  mutate(Index_Name = str_replace(Index_Name, "^(.*?)(\\|.*$)", "\\1"),
         Name = str_replace(Name, "^(.*?)(\\|.*$)", "\\1"),
         Given_Name = str_replace(Given_Name, "^(.*?)(\\|.*$)", "\\1"),
         Pseudonym_Kryptonym = str_replace(Pseudonym_Kryptonym, "^(.*?)(\\|.*$)", "\\1"),
         Project_ID = as.integer(str_replace(Project_ID, "^(.*?)(\\|.*$)", "\\1")),
         Death = as.integer(Death)) %>% 
  select(-Freq,-pierwsza_litera)
class(te_same_osoby$Project_ID)
nodegoat_people <- nodegoat_people %>% 
  bind_rows(te_same_osoby)

nodegoat_people <- nodegoat_people
for (i in 1:nrow(nodegoat_people)) {
  nodegoat_people$match[i] <- grepl(nodegoat_people$Name[i], nodegoat_people$viaf_name[i])
  
}

nodegoat_people$LOD_ID[nodegoat_people$match == FALSE] <- NA

nodegoat_people <- nodegoat_people %>% 
  mutate(Pseudonym_Kryptonym = ifelse(Pseudonym_Kryptonym == "", "; ", as.character(Pseudonym_Kryptonym))) %>% 
  cSplit("Pseudonym_Kryptonym",sep = "; ",direction = "long") %>% 
  unique() %>% 
  mutate(Pseudonym_Kryptonym = str_remove(Pseudonym_Kryptonym, ";"))

nodegoat_people[nodegoat_people==""]<-NA
nodegoat_people <- nodegoat_people %>% 
  group_by(Project_ID) %>%
  summarise_each(funs(paste(unique(na.omit(.)), collapse = "|"))) %>% 
  mutate(True_Name = ifelse(True_Name == "", "|", as.character(True_Name))) %>% 
  cSplit("True_Name",sep = "|",direction = "long") %>% 
  unique()
nodegoat_people[nodegoat_people==""]<-NA
nodegoat_people <- nodegoat_people %>% 
  group_by(Project_ID) %>%
...
nodegoat_people$Unknown_Name_Source[!grepl("sam|smz", nodegoat_people$Unknown_Name_Source)] <- ""

nodegoat_people <- nodegoat_people %>% 
  mutate(Pseudonym_Kryptonym = str_replace_all(Pseudonym_Kryptonym, "\\|", "; "),
         True_Name = str_replace_all(True_Name, "\\|", "; "),
         Other_Name_Form = str_replace_all(Other_Name_Form, "\\|", "; "),
         Addition_to_Name = str_replace_all(Addition_to_Name, "\\|", "; ")) %>% 
  select(-te_same_osoby, -match)
...
kartoteka_instytucji <- sheets_read(ss = "1MX8oqQly65GzkEhwKGf7NqBc4L4B5mNj12NtgLgB5Bs", sheet = "Arkusz1",col_types = "c") %>% 
  mutate(Entity_Name = ifelse(str_count(Entity_Name, "\\[") == 1 & str_count(Entity_Name, "\\]") == 0, trim2(paste(Entity_Name, "]", sep = "")),
                              ifelse(str_count(Entity_Name, "\\[") == 0 & str_count(Entity_Name, "\\]") == 1 & substr(Entity_Name, nchar(Entity_Name), nchar(Entity_Name)) == "]", trim2(str_remove(Entity_Name, "\\]")), trim2(as.character(Entity_Name)))),
         Entity_Name = ifelse(substr(Entity_Name, 1, 1) == "\"" & substr(Entity_Name, nchar(Entity_Name), nchar(Entity_Name)) == "\"", str_remove_all(Entity_Name, "\""), as.character(Entity_Name)),
         Entity_Name = ifelse(substr(Entity_Name, 1, 1) == "\"" & str_count(Entity_Name, "\"") == 1, str_remove(Entity_Name, "\""), as.character(Entity_Name)),
         Entity_Name = str_remove_all(Entity_Name, "^\\[\"|\"\\]$"),
         Entity_Name = ifelse(str_count(Entity_Name, "\\[") == 1 & str_count(Entity_Name, "\\]") == 1 & substr(Entity_Name, 1, 1) == "[" & substr(Entity_Name, nchar(Entity_Name), nchar(Entity_Name)) == "]", str_remove_all(Entity_Name, "^\\[|\\]$"), as.character(Entity_Name)),
         Entity_Name = str_remove_all(Entity_Name, "~\\d+|~\\:|~\\,"),
         Entity_Name = str_remove_all(Entity_Name, "~"),
         Located_Location = ifelse(str_count(Located_Location, "\\[") == 1 & str_count(Located_Location, "\\]") == 0, paste(Located_Location, "]", sep = ""), as.character(Located_Location)),
         Located_Location = str_remove_all(Located_Location, "~\\d+|~\\:|~\\,"),
         Located_Location = str_remove_all(Located_Location, "~|\\] \\:$| \\:$"))
...
nodegoat_institutions <- nodegoat_institutions %>% 
  ...
  mutate(Related_Entity_Sub_Entity = ifelse(Related_Entity_Sub_Entity!="",as.character(Project_ID_sub),""),
         Related_Entity_Main_Entity = ifelse(Related_Entity_Main_Entity!="",as.character(Project_ID_main),"")) %>% 
...
kartoteka_wydarzen <- sheets_read(ss = "1BosLyDVa7uxNTxw3Lt19PPADx8ZZcEuLYDnl--6vVDM", sheet = "Arkusz1",col_types = "c") %>% 
  mutate(Event_Name = trim(trim2(Event_Name)),
         Event_Other_Names = trim(trim2(Event_Other_Names)),
         Event_Number = trim(trim2(Event_Number)),
         Event_Number_Arabic = trim(trim2(Event_Number_Arabic)),
         Event_Year = trim(trim2(Event_Year)),
         Located = trim(trim2(Located)),
         Organiser_Main = trim(trim2(Organiser_Main)),
         Organiser_Sub = trim(trim2(Organiser_Sub)))
...
write.csv2(nodegoat_events, file = "C:/Users/Cezary/Desktop/nodegoat_events.csv", row.names = F, na = '', fileEncoding = 'UTF-8', quote = FALSE)

#serie

kartoteka_serii <- sheets_read(ss = "1UPBWh8743mJWVGFD1fg3CHx03eOH8lSWcX090QIFJgs", sheet = "Arkusz1",col_types = "c") %>% 
  mutate(Series_Title = trim(trim2(Series_Title)),
         Addition_to_Title = trim(trim2(Addition_to_Title)),
         Creator = trim(trim2(Creator)),
         Sub_Series = trim(trim2(Sub_Series)),
         Main_Series = trim(trim2(Main_Series)))
...
nodegoat_series <- kartoteka_serii  %>% 
  ...
  unique() %>% 
  mutate(Project_ID = sprintf("%05d", 1:n())) %>% 
  mutate(Sub_Series = ifelse(Sub_Series == "", "|", as.character(Sub_Series)),
         Main_Series = ifelse(Main_Series == "", "|", as.character(Main_Series))) %>% 
  cSplit("Series_Title",sep = "|",direction = "long") %>% 
  cSplit("Sub_Series",sep = "|",direction = "long") %>% 
  cSplit("Main_Series",sep = "|",direction = "long") %>% 
  filter(!(is.na(Series_Title) & is.na(Sub_Series) & is.na(Main_Series)))
  
nodegoat_series <- nodegoat_series %>% 
  left_join(nodegoat_series %>% select(Project_ID_sub = Project_ID, Series_Title, Main_Series), by = c("Sub_Series" = "Series_Title", "Series_Title" = "Main_Series"))
nodegoat_series <- nodegoat_series %>% 
  left_join(nodegoat_series %>% select(Project_ID_main = Project_ID, Series_Title, Sub_Series), by = c("Main_Series" = "Series_Title", "Series_Title" = "Sub_Series")) %>% 
  mutate(Sub_Series = ifelse(Sub_Series!="",as.character(Project_ID_sub),""),
         Main_Series = ifelse(Main_Series!="",as.character(Project_ID_main),"")) %>% 
  select(-Project_ID_sub, -Project_ID_main)
  
nodegoat_series[nodegoat_series==""] <- NA  
nodegoat_series <- nodegoat_series %>% 
  group_by(Project_ID) %>% 
  summarise_each(funs(paste(unique(na.omit(.)), collapse = "|")))
  
write.csv2(nodegoat_series, file = "C:/Users/Cezary/Desktop/nodegoat_series.csv", row.names = F, na = '', fileEncoding = 'UTF-8', quote = FALSE)
...
nodegoat_series <- nodegoat_series %>% 
  ...
  cSplit("Main_Series",sep = "|",direction = "long") %>% 
  cSplit("Sub_Series",sep = "|",direction = "long") %>% 
  filter(!is.na(Sub_Series)|!is.na(Main_Series)) %>% 
  mutate(Sub_Series = ifelse(is.na(Sub_Series), NA, sprintf("%05d", Sub_Series)),
         Main_Series = ifelse(is.na(Main_Series), NA, sprintf("%05d", Main_Series)))

write.csv2(nodegoat_series, file = "C:/Users/Cezary/Desktop/nodegoat_series_relations.csv", row.names = F, na = '', fileEncoding = 'UTF-8', quote = FALSE)

```

```{r ksiazki do nodegoata}
#title
#bn books
write.csv2(bn_books, file = "C:/Users/Cezary/Desktop/bn_books.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
write.csv2(cz_books, file = "C:/Users/Cezary/Desktop/cz_books.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
write.csv2(cz_articles, file = "C:/Users/Cezary/Desktop/cz_articles.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
write.csv2(pbl_books, file = "C:/Users/Cezary/Desktop/pbl_books.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
write.csv2(pbl_articles, file = "C:/Users/Cezary/Desktop/pbl_articles.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
```
