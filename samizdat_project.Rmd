...
options(scipen = 999)
pacman::p_load(stringr,tidyverse,RJDBC,sqldf,dplyr,splitstackshape,svMisc,data.table,googlesheets4)
...
kartoteka_osob <- sheets_read(ss = "1Y8Y_pfkuKiv5npL6QJAXWbDO6twIJqsK3ArvaokjFkU", sheet = "samizdat") %>% 
  select(-test) %>% 
...
blabla <- record_people %>% 
  unique() %>% 
  left_join(kartoteka_osob %>% select(name_form_id,group_id),by="name_form_id") %>% 
  filter(!is.na(group_id)) %>% 
  mutate(name_form = str_remove(name_form,"(, NA)+"),
         record_id = as.integer(record_id)) %>% 
  left_join(bind_rows(marc_field_bnb_100,marc_field_bnb_700,marc_field_bnb_701) %>% select(-X100,-X700,-X701),c("record_id"="id")) %>% 
  left_join(bind_rows(marc_field_czb_100,marc_field_czb_600,marc_field_czb_700,marc_field_cza_100,marc_field_cza_600,marc_field_cza_700) %>% select(-X100,-X600,-X700),c("record_id"="id")) %>% 
  mutate(Project_ID = ifelse(data_set=="bn_books",1,
                             ifelse(data_set=="cz_books",2,
                                    ifelse(data_set=="cz_articles",3,
                                           ifelse(data_set=="pbl_books",4,5)))),
         LOD_ID = NA,
         Index_Name = case_when(
           data_set == "bn_books" ~ unite("Index_Name",`%1`,`%2`,`%3`,`%4`,`%5`,`%6`,sep = " ",na.rm=TRUE),
           data_set %in% c("cz_books","cz_articles") ~ unite(`$$a`,na.rm=TRUE),
           TURE ~ as.character(name_form)
         ))

  if(blabla$data_set=="bn_books") {
    unite(blabla,"Index_Name",`%1`,`%2`,`%3`,`%4`,`%5`,`%6`,sep = " ",na.rm=TRUE)
  }
...
,
         Index_Name = ifelse(data_set=="bn_books",str_remove(paste(`%1`,`%2`,sep = ", "),", $"),
                             ifelse(data_set %in% c("cz_books","cz_articles"),`$$a`,as.character(name_form))),
         Name = ifelse(data_set=="bn_books",`%1`,
                             ifelse(data_set %in% c("cz_books","cz_articles"),str_replace(`$$a`,"(.*?)(,.*$)","\\1"),as.character(str_replace(name_form,"(.*?)(,.*$)","\\1")))),
         Given_Name = ifelse(data_set=="bn_books",`%2`,
                             ifelse(data_set %in% c("cz_books","cz_articles"),str_replace(`$$a`,"(.*?, )(.*$)","\\2"),as.character(str_replace(name_form,"(.*?, )(.*$)","\\2"))))) %>% 
  unite("Pseudonyms",`%p`:`%k`,sep = "; ",na.rm=TRUE) %>% 
  mutate(True_Name = `%w`,
         Other_Name_Form = `%s`)
...
test <- kartoteka_osob %>% 
  select(name_form_id_1) %>% 
  unique()

kartoteka_instytucji <- sheets_read(ss = "1MX8oqQly65GzkEhwKGf7NqBc4L4B5mNj12NtgLgB5Bs", sheet = "Arkusz1")

kartoteka_serii <- sheets_read(ss = "1UPBWh8743mJWVGFD1fg3CHx03eOH8lSWcX090QIFJgs", sheet = "Arkusz1")

kartoteka_wydarzen <- sheets_read(ss = "1BosLyDVa7uxNTxw3Lt19PPADx8ZZcEuLYDnl--6vVDM", sheet = "Arkusz1")
