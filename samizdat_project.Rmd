...
ggg <- bind_rows(marc_field_bnb_100,marc_field_bnb_700,marc_field_bnb_701)
...
xxx <- bind_rows(marc_field_czb_100,marc_field_czb_600,marc_field_czb_700,marc_field_cza_100,marc_field_cza_600,marc_field_cza_700)
...
for (i in x) {
  progress(match(i,x), max.value = length(x))
  test$person_match[i] <- ifelse(grepl(test$`$$a`[i],test$name_form[i]),TRUE,FALSE)
}

test <- test %>% 
  filter(is.na(person_match)|person_match==TRUE) %>% 
  select(-person_match)

x <- 1:nrow(test)
for (i in x) {
  if (!is.na(test$subfield[i])) {
    progress(match(i,x), max.value = length(x))
    abc <- test[colnames(test)==test$subfield[i]]
    test$person_match[i] <- ifelse(grepl(abc[i,],test$name_form[i]),TRUE,FALSE)
  }
}
...
for (i in x) {
  if (!is.na(test$`%2`[i])) {
    progress(match(i,x), max.value = length(x))
    test$person_match2[i] <- ifelse(grepl(paste3(test$`%1`[i],test$`%2`[i],sep = " "),test$name_form[i]),TRUE,FALSE)
  }
}

test2 <- test %>% 
  mutate(person_match2 = ifelse(subfield!="%1"&person_match==TRUE&person_match2==FALSE,TRUE,person_match2))

dobre <- test2 %>% 
  filter(!(is.na(`%2`)&is.na(person_match)&person_match2==TRUE)&!(person_match==TRUE&person_match2==FALSE)&!(person_match==FALSE&person_match2==FALSE)&!(person_match==FALSE&person_match2==TRUE))

blabla <- test %>% 
  filter(person_match==TRUE&person_match2==FALSE)


test2 <- test %>% 
  mutate(person_match2 = ifelse(subfield=="%1",person_match2,TRUE)) %>% 
  filter(is.na(person_match)|person_match==TRUE,
         is.na(person_match2)|person_match2==TRUE)
...
%>% 
  group_by(LOD_ID) %>% 
  mutate(Project_ID = paste(unique(na.omit(Project_ID)),collapse = "|"),
         Index_Name = paste(unique(na.omit(Index_Name)),collapse = "|"),
         Name = paste(unique(na.omit(Name)),collapse = "|"),
         Given_Name = paste(unique(na.omit(Given_Name)),collapse = "|"),
         Pseudonym_Kryptonym = paste(unique(na.omit(Pseudonym_Kryptonym)),collapse = "|"),
