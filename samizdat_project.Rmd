...
nodegoat_people <- nodegoat_people %>% 
  mutate(ile_znakow = nchar(Index_Name))


test <- nodegoat_people
for (i in 1:nrow(nodegoat_people)) {
  test$match[i] <- grepl(test$Name[i], test$viaf_name[i])
  
}

count <- as.data.frame(table(test$match))
    


#write.xlsx(nodegoat_people, "C:/Users/Cezary/Desktop/nodegoat_people.xlsx")
write.csv2(nodegoat_people, file = "C:/Users/Cezary/Desktop/nodegoat_people_viaf_licznik.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
